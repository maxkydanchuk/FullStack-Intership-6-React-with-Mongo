export default class apiService {
  ...
  getAllPeople = async (param, query , value ) => {
    ...
    if(param && query && value) {
      res = await this.getResource(`/people?sortOrder=${param}&sortBy=${query}&search=${value}`)
    }
    else if(param && query) {
       res = await this.getResource(`/people?sortOrder=${param}&sortBy=${query}`)
    } 
    else if(value) {
      res = await this.getResource(`/people?search=${value}`)
    }  else {
      res = await this.getResource(`/people/`)
    }
