  ...
  getAllPeople = async (param, query , value ) => {
    ...
    if(param && query && value) {
      res = await this.getResource(`/people?page=1&size=10&sortOrder=${param}&sortBy=${query}&search=${value}`)
       ...
       res = await this.getResource(`/people?page=1&size=10&sortOrder=${param}&sortBy=${query}&page=1&size=3`)
    } 
    else if(value) {
      res = await this.getResource(`/people?page=1&size=10&search=${value}&page=1&size=3`)
    }  else {
