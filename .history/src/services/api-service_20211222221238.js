export default class apiService {
  ...
  getAllPeople = async (param, query , value ) => {
    ...
    if(param && query && value) {
      res = await this.getResource(`/people?sortOrder=${param}&sortBy=${query}&search=${value}&page=1&size=10`)
    }
    else if(param && query) {
       res = await this.getResource(`/people?sortOrder=${param}&sortBy=${query}&page=1&size=3`)
    } 
    else if(value) {
      res = await this.getResource(`/people?search=${value}&page=1&size=3`)
    }  else {
